 ...
 *   string json;
 *   SwarmJSON.Serialize(myObject, json);
 * 
 * Deserialize a JSON string into a typed object:
 *   string error;
 *   SwarmJSON.Deserialize(jsonString, myObject, error);
 * 
 * Parse a JSON string into a dynamic node tree:
 *   SwarmJsonNode root = SwarmJSON.Parse("{\"name\": \"test\", \"value\": 42}");
 *   string name = root.Get("name").GetString();   // "test"
 *   int value   = root.Get("value").GetInt();      // 42
 * 
 * Convert a node tree back to a JSON string:
 *   string json = SwarmJSON.Stringify(root);           // compact
 *   string pretty = SwarmJSON.Stringify(root, true);   // pretty-printed
 * 
 * Escape a string for safe embedding in JSON:
 *   string safe = SwarmJSON.Escape(playerName);
...
class SwarmJSON
{
	...
	static bool Serialize(Class obj, out string result, bool pretty = true)
	{
		...
		if (!success)
			Print("[SwarmJSON] Serialize failed for " + obj.ClassName());
	...
	static bool Deserialize(string json, Class obj, out string error)
	{
		...
		if (!success)
			Print("[SwarmJSON] Deserialize failed: " + error);

		return success;
	}

	// ---------------------------------------------------------------
	// Parse (string → SwarmJsonNode tree)
	// ---------------------------------------------------------------

	/**
	 * Parse a JSON string into a dynamic SwarmJsonNode tree.
	 * Returns null if the JSON is invalid (error is printed to log).
	 * 
	 * @param json  The JSON string to parse
	 * @return Root node of the parsed tree, or null on error
	 */
	static SwarmJsonNode Parse(string json)
	{
		if (json == "")
			return null;

		SwarmJsonParser parser = new SwarmJsonParser();
		return parser.Parse(json);
	}

	// ---------------------------------------------------------------
	// Stringify (SwarmJsonNode tree → string)
	// ---------------------------------------------------------------

	/**
	 * Convert a SwarmJsonNode tree to a JSON string.
	 * 
	 * @param node    The root node to stringify
	 * @param pretty  If true, output is indented with 2 spaces (default: false)
	 * @return The JSON string
	 */
	static string Stringify(SwarmJsonNode node, bool pretty = false)
	{
		if (!node)
			return "null";
	...
	protected static string StringifyNode(SwarmJsonNode node, bool pretty, int depth)
	{
		switch (node.GetType())
		{
	...
	protected static string StringifyArray(SwarmJsonNode node, bool pretty, int depth)
	{
	...
	protected static string StringifyObject(SwarmJsonNode node, bool pretty, int depth)
	{
		...
		for (int i = 0; i < keys.Count(); i++)
		{
			...
			SwarmJsonNode value = node.Get(key);
