...
class SwarmJsonParser
{
	...
	SwarmJsonNode Parse(string json)
	{
		m_Json = json;
		m_Pos = 0;
		m_Len = json.Length();
		...
		SwarmJsonNode result = ParseValue();

		if (m_Error != "")
		{
			Print("[SwarmJSON] Parse error: " + m_Error);
			return null;
		}
	...
	protected SwarmJsonNode ParseValue()
	{
		SkipWhitespace();
	...
	protected SwarmJsonNode ParseString()
	{
		string value = ParseStringRaw();
		if (m_Error != "")
			return null;

		return SwarmJsonNode.NewString(value);
	}
	...
	protected SwarmJsonNode ParseNumber()
	{
		int startPos = m_Pos;
		bool isFloat = false;
		...
		if (isFloat)
			return SwarmJsonNode.NewFloat(numStr.ToFloat());
		else
			return SwarmJsonNode.NewInt(numStr.ToInt());
	}
	...
	protected SwarmJsonNode ParseBool()
	{
		if (m_Pos + 4 <= m_Len && m_Json.Substring(m_Pos, 4) == "true")
		{
			m_Pos += 4;
			return SwarmJsonNode.NewBool(true);
		}

		if (m_Pos + 5 <= m_Len && m_Json.Substring(m_Pos, 5) == "false")
		{
			m_Pos += 5;
			return SwarmJsonNode.NewBool(false);
		}
	...
	protected SwarmJsonNode ParseNull()
	{
		if (m_Pos + 4 <= m_Len && m_Json.Substring(m_Pos, 4) == "null")
		{
			m_Pos += 4;
			return SwarmJsonNode.NewNull();
		}
	...
	protected SwarmJsonNode ParseObject()
	{
		// Skip opening brace
		m_Pos++;
		...
		while (m_Pos < m_Len)
		{
			...
			// Parse value
			SkipWhitespace();
			SwarmJsonNode value = ParseValue();
			if (m_Error != "")
				return null;
	...
	protected SwarmJsonNode ParseArray()
	{
		// Skip opening bracket
		m_Pos++;

		SwarmJsonNode arr = SwarmJsonNode.NewArray();
