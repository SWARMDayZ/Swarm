	...
	{        
		...
		if ( GetGame().GetInput().LocalPress("UAEarPlugsToggle") && GetGame().GetUIManager().GetMenu() == NULL ) 
		{
			...
			if (m_EarPlugsState == 0) {  // If we are at state 0 we go to state 1 which is 45% volume
				...
				m_earplugswidget.SetIcon("Swarm\\data\\volume_mid.edds");
				...
				//Calling our Function to change the icon
				m_earplugswidget.SetIcon("Swarm\\data\\volume_low.edds");
				...
				//Calling our Function to change the icon
				m_earplugswidget.SetIcon("Swarm\\data\\volume_off.edds");
				...
				//Calling our Function to change the icon
				m_earplugswidget.SetIcon("Swarm\\data\\volume_full.edds");
