...
set SOURCE_DIR=%~dp0src\SwarmSpectator
set OUTPUT_DIR=%~dp0dist\@SwarmSpectator\Addons
set TEMP_DIR=%~dp0.build_temp\SwarmSpectator
...
if %ERRORLEVEL% EQU 0 (
    echo SwarmSpectator built successfully!
) else (
    echo SwarmSpectator build failed with error code %ERRORLEVEL%
)
